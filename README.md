...
- 🤡 Главная фича - падает ряд оригинальных тестов
- Убрана mTLS аутентификация, в данный момент клиент-серверная коммуникация происходит по HTTP
- Логика обработки конфига вынесена в класс ConfigManager
- В API добавлены экшены для изменения конфигурации без перезагрузки сервиса
  - `updateCerts` - добавление сертификатов
  - `deleteCert` - удаление сертификатов
  - `getSettings` - получение текущих настроек сервиса

### Работа с API

Сервис запущенный в serve режиме доступен на порту `4433`

Вся работа по api происходит через POST запросы по адресу `/`

Тело запроса должно быть в формате JSON и содержать параметр `action`

Все остальные параметры в запросе будут интерпретированы как payload для экшена
...
Если сертификата с таким именем нет, то он будет добавлен, если есть - он будет обновлен
...
{
  "success": false,
  "error": "Certificate test not found"
}
```
